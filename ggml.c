...
inline static void ggml_vec_leaky_f32(const int n, float *y, const float *x) {
    for (int i = 0; i < n; ++i) y[i] = (x[i] > 0.f) ? x[i] : 0.1f * x[i];
}
            ...
            for (int64_t i02 = 0; i02 < ne02; i02++) {
                ...
                for (int64_t i01 = ir0; i01 < ir1; i01++) {
                    for (int64_t i00 = 0; i00 < ne00; i00++) {
                        const char *src0_ptr = ((char *) src0->data + i00 * nb00 + i01 * nb01 + i02 * nb02 + i03 *
                                                nb03);
        ...
        for (int i2 = ith; i2 < ne2; i2++) {
            if (i2 < ne02) {
                ...
                for (int i1 = 0; i1 < ne1; i1++) {
                    for (int i0 = 0; i0 < ne0; i0++) {
                        const float *x = (float *) (
                            (char *) src0->data + i0 * nb00 + i1 * nb01 + i2 * nb02 + i3 * nb03);
        ...
        for (int i02 = ith; i02 < ne02; i02++) {
            for (int m = 0; m < dst->ne[1]; m++) {
                ...
                for (int n = 0; n < dst->ne[0]; n++) {
                    ...
                    const float *x = (float *) ((char *) src0->data + i00 * nb00 + i01 * nb01 + i02 * nb02 + i03 *
                                                nb03);

                    float *y = (float *) ((char *) dst->data + n * dst->nb[0] + m * dst->nb[1] + i02 * dst->nb[2] + i03
                                          * dst->nb[3]);
            ...
            for (int iq1 = 0; iq1 < neq1; ++iq1) {
                ...
                for (int64_t ic = 0; ic < D; ++ic) {
                    ggml_vec_mad_f32(masked_begin,
                                     ...
                                     (float *) ((char *) v->data + (ic * nbv1 + iv2 * nbv2 + iv3 * nbv3)),
                                     *(float *) (
                                         (char *) d->data + (ic * nbd0 + id1 * nbd1 + id2 * nbd2 + id3 * nbd3)));
                }
...
static void ggml_compute_forward_mul_mat_sparse(
    ...
    const size_t predictor_row_size = dst->src[2]->ne[0] * ggml_type_size(GGML_TYPE_F32) /
                                      ggml_blck_size(GGML_TYPE_F32);
#ifdef DI_STATISTICS
    struct ggml_tensor *statistics_tensor = dst->src[4];
    size_t statistics_row_size = 0;
    if (statistics_tensor) {
                ...
                for (int64_t ir1 = iir1; ir1 < iir1 + blck_1 && ir1 < ir111; ++ir1) {
                    ...
                    const char *src1_col = (const char *) wdata +
                                           (src1_cont || src1->type != vec_dot_type
                                                ? (i11 + i12 * ne11 + i13 * ne12 * ne11) * row_size
                                                : (i11 * nb11 + i12 * nb12 + i13 * nb13));
                    ffdata = (float *) ((char *) predictor_data + (i11 + i12 * ne11 + i13 * ne12 * ne11) *
                                        predictor_row_size);
...
// vz = alpha * vx + vy
static void ggml_axpy_normal_f16(const int n, const ggml_fp16_t *vx, const ggml_fp16_t *restrict vy, void *restrict vz,
                                 ggml_fp16_t alpha) {
    float *res = (float *) vz;
    for (int i = 0; i < n; i++) {
...
static void ggml_axpy_avx_f16(const int n, const ggml_fp16_t *restrict vx, const ggml_fp16_t *vy, void *vz,
                              ggml_fp16_t alpha) {
#if defined(__AVX2__)
...
static void ggml_graph_compute_perf_stats_node_gpu(struct ggml_tensor *node,
...
void ggml_graph_print(const struct ggml_cgraph *cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        GGML_PRINT(
            ...
            "] %16s %48s %s (%3d) cpu = %7.3f / %7.3f ms, wall = %7.3f / %7.3f ms\n",
            i,
